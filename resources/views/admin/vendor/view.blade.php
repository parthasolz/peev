<x-adminlayout title="Vendor| View">
                            ...
                            <div class="card-body">
                                <div class="d-flex">
                                    ...
                                    <div class="flex-grow-1 overflow-hidden">                                      
                                        <h5 class="text-truncate font-size-15">{{ $vendor_detail->name }}</h5>
                                        <p>{{ $vendor_detail->email }}<br>
                                        {{ $vendor_detail->phone }}
                                    </div>
                                    <div class="page-title-right">
                                        <a href="{{ route('admin.vendor.edit',$vendor_detail->id) }}" class="btn btn-outline-secondary btn-sm">Update</a>
                                        @if($vendor_detail->status==1)
                                        <a href="{{ route('admin.project.create',['id'=>$vendor_detail->id]) }}" class="btn btn-outline-secondary btn-sm">Create Project</a>
                                        @endif
                                </div>
                                </div>
                                <h5 class="font-size-15 mt-4">Address :</h5>
                                @if($vendor_detail->address->count()>0)
                                @foreach ($vendor_detail->address as $add)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="table-responsive">
                                    <h5>{{ $vendor_detail->name }}&nbsp;Projects</h5>
                                    <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
                                        ...
                                        <tbody>
                                            @foreach($user_project as $project_user)
                                            <tr>
